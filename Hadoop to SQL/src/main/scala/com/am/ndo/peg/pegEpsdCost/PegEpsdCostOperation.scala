...
class PegEpsdCostOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	def loadData(): Map[String, DataFrame] = {
			...
			val startTime = DateTime.now
					...
					val pegCtgryCdQuery = config.getString("query_peg_ctgry_cd").replaceAll(ConfigKey.sourceDBPlaceHolder, inboundHiveDB).toLowerCase()
					val mlsaZipCdQuery = config.getString("query_mlsa_zip_cd").replaceAll(ConfigKey.sourceDBPlaceHolder, inboundHiveDB).toLowerCase()
					val apiProvWrkQuery = config.getString("query_api_prov_wrk1").replaceAll(ConfigKey.stagingDBPlaceholder, stagingHiveDB).toLowerCase()
					...
					println(s"[NDO-ETL] Query for reading data from etg_base_cls_cd table is $pegCtgryCdQuery")
					...
					val pegCtgryCdDf = spark.sql(pegCtgryCdQuery)
					...
					val mapDF = Map("pcr_cost_epsd" -> pcrCostEpsdDf, "fncl_prod_cf" -> fnclProdCfDf, "ndo_zip_submarket_xwalk" -> ndoZipSubmarketXwalkDf,
							...
							"fncl_mbu_cf" -> fnclMbuCfDf, "peg_ctgry_cd" -> pegCtgryCdDf, "mlsa_zip_cd" -> mlsaZipCdDf, "api_prov_wrk" -> apiProvWrkDf,"ndowrk_ratg_addr" -> ndowrkRatgAddrDf,
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			val startTime = DateTime.now
					...
					val pegCtgryCdDf = inMapDF.getOrElse("peg_ctgry_cd", null)
					...
					val tabPegEpsdCost = config.getString("tab_peg_epsd_cost")
					val tabPegEpsdCostBkp = config.getString("tab_peg_epsd_cost_bkp")

					val maxrun_id = getMAxRunId(tabPegEpsdCost)
					...
					val f = for {
						...
						ndoWrkEpsdCost2Df <- ndoWrkEpsdCost2(pcrCostEpsdDf, fnclProdCfDf, ndoZipSubmarketXwalkDf, pcrEtgMbrSmryDf, fnclMbuCfDf, pegCtgryCdDf, mlsaZipCdDfFil ,apiProvWrkDfFil, snapNbr, ndowrkRatgAddrDf,pcrEtgSmryFilDf)
//						//TODO Delete the write to table
//						WriteNdoWrkEpsdCost2 <- WriteSaveAsTableOverwrite(ndoWrkEpsdCost2Df, stagingHiveDB, tabNdoWrkEpsdCost2)
						...
						WriteMlrBenPlan <- WriteManagedTblInsertOverwrite(ndoWrkEpsdCostDfWc, warehouseHiveDB, tabPegEpsdCost)
						WriteapiMlrSmryWC <- WriteExternalTblAppend(ndoWrkEpsdCostDfWc, warehouseHiveDB, tabPegEpsdCostBkp)

					} yield {
						println("Episode Cost - Monad execution completed!")
					}
					f.run(spark)
					runIDInsert(tabPegEpsdCost, maxrun_id)
