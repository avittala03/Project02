...
case class ndoWrkEpsdCost2(pcrCostEpsdDf: Dataset[Row], fnclProdCfDf: Dataset[Row], ndoZipSubmarketXwalkDf: Dataset[Row],
		pcrEtgMbrSmryDf: Dataset[Row], fnclMbuCfDf: Dataset[Row], pegCtgryCdDf: Dataset[Row], mlsaZipCdDfFil: Dataset[Row], apiProvWrkDfFil: Dataset[Row],
...
//and trim(col("pcrCostEpsdDf1.PEER_MRKT_CD")) filter change to single peer_mrkt_cd
			...
			&& (trim(pcrCostEpsdDf("NTWK_ST_CD")).isin("CA","NY", "VA") && trim(pcrCostEpsdDf("PEER_MRKT_CD")).isin("20")
					|| trim(pcrCostEpsdDf("NTWK_ST_CD")).isin("CA", "NY", "VA") === false && trim(pcrCostEpsdDf("PEER_MRKT_CD")).isin("19")))
			...
			val pcrApiJoinPcrSmry = pcrJoinApi.join(pegCtgryCdDf, trim(pegCtgryCdDf("PEG_CTGRY_CD")) === trim(pcrCostEpsdDf("ETG_BASE_CLS_CD")), "left_outer")
...
//and trim(col("pcrCostEpsdDf1.PEER_MRKT_CD")) filter change to single peer_mrkt_cd
			...
			val finalSel = pcrApiJoinPcrSmry.select(
					...
					coalesce(pegCtgryCdDf("cd_val_nm"), lit("NOTF")).alias("ETG_BASE_CLS_DESC"),
