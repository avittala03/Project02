...
class ApiPegGrpOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	val tabApiPegGrp = config.getString("tab_api_peg_grp")
	val tabApiPegGrpBkp = config.getString("tab_api_peg_grp_bkp")
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			val startTime = DateTime.now
					...
					val maxrun_id_api_etg_grp = getMAxRunId(tabApiPegGrp)
					...
					val pcrGrpEfcncyRatioMultiSpltDfFil = pcrGrpEfcncyRatioMultiSpltDf.filter(trim(pcrGrpEfcncyRatioMultiSpltDf("GRP_AGRGTN_TYPE_CD")) === "TIN" 
					&& trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("UNK", "NA") === false
					...
					&& ((trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("CA", "NY", "VA") && trim(pcrGrpEfcncyRatioMultiSpltDf("PEER_MRKT_CD")).isin("20"))
							|| (trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("CA", "NY", "VA") === false && trim(pcrGrpEfcncyRatioMultiSpltDf("PEER_MRKT_CD")).isin("19"))
							))
					...
					val apiProvWrkPcrGrpJoin = pcrGrpEfcncyRatioMultiSpltDfFil.join(apiProvWrkDfDist,
							...
							&& trim(pcrGrpEfcncyRatioMultiSpltDfFil("GRP_AGRGTN_ID")) ===  trim(apiProvWrkDfDist("PROV_TAX_ID"))
					...
					writeTblInsertOverwrite(apiEtgGrpDf, warehouseHiveDB, tabApiPegGrp)
					writeTblAppendOverwrite(apiEtgGrpDf, warehouseHiveDB, tabApiPegGrpBkp)

					runIDInsert(tabApiPegGrp, maxrun_id_api_etg_grp)
