...
class ApiPegGroupSpcltyOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	val tabApiPegGroupSpclty = config.getString("tab_api_peg_group_spclty")
	val tabApiPegGroupSpcltyBkp = config.getString("tab_api_peg_group_spclty_bkp")
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			val startTime = DateTime.now
					...
					val maxrun_id = getMAxRunId(tabApiPegGroupSpclty)
					println(s"[NDO-ETL] Max run id is : $maxrun_id")

					val snapNbr = getSnapNbr(inMapDF)
					println(s"[NDO-ETL] Available snapNbr is : $snapNbr")

					val pcrGrpEfcncyRatioDfFilter = pcrGrpEfcncyRatioDf.filter(trim(pcrGrpEfcncyRatioDf("GRP_AGRGTN_TYPE_CD")) === "TIN" 
					&&  trim(pcrGrpEfcncyRatioDf("NTWK_ST_CD")).isin("UNK", "NA") === false &&
					pcrGrpEfcncyRatioDf("BKBN_SNAP_YEAR_MNTH_NBR") === snapNbr &&
					pcrGrpEfcncyRatioDf("GRP_DSTNCT_EPSD_CNT") >= 20 &&
					((trim(pcrGrpEfcncyRatioDf("NTWK_ST_CD")).isin("CA", "NY", "VA") && trim(pcrGrpEfcncyRatioDf("PEER_MRKT_CD")).isin("20"))
							|| (trim(pcrGrpEfcncyRatioDf("NTWK_ST_CD")).isin("CA", "NY", "VA") === false && trim(pcrGrpEfcncyRatioDf("PEER_MRKT_CD")).isin("19"))
							 ))
					...
					val apiProvWrkPcrGrpEfcncyRatioJoin = pcrGrpEfcncyRatioDfFilter.join(apiProvWrkDfDist,
							...
							trim(pcrGrpEfcncyRatioDfFilter("GRP_AGRGTN_ID")) ===  trim(apiProvWrkDfDist("PROV_TAX_ID"))	 &&
							...
							trim(apiProvWrkDfDist("prod_id")) === (when(trim(pcrGrpEfcncyRatioDfFilter("BNCHMRK_PROD_DESC")) === "NO GROUPING", lit("ALL"))), "inner")
					...
					writeTblInsertOverwrite(apiEtgGroupSpcltyDf, warehouseHiveDB, tabApiPegGroupSpclty)
					writeTblAppendOverwrite(apiEtgGroupSpcltyDf, warehouseHiveDB, tabApiPegGroupSpcltyBkp)

					runIDInsert(tabApiPegGroupSpclty, maxrun_id)
