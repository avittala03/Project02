...
class ProfReferralPatternsOperationF(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			...
			val f = for {
							...
							////-------------------------------- File f -------------------------------------------
				yueProfRenNpi <- yueProfRenNpi(clmProv, clmLineProv, tabProf, tabProfPdlNpi, stagingHiveDB)
				WriteyueProfBilNpi <- WriteManagedTblInsertOverwrite(yueProfRenNpi, stagingHiveDB, tabProfRenNpi)

				yueProfRenTax <- yueProfRenTax(clmProv, clmLineProv, tabProf, tabProfPdlTax, stagingHiveDB)
				WriteyueProfBilNpi <- WriteManagedTblInsertOverwrite(yueProfRenTax, stagingHiveDB, tabProfRenTax)
