...
class ProfReferralPatternsOperationD(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			...
			val f = for {
				...
				////-------------------------------- File d -------------------------------------------
				ProfRefNpi <- ProfRefNpi(clmProv, clmLineProv, tabProf, tabProfPdlNpi, stagingHiveDB)
				WriteyueProfDF <- WriteManagedTblInsertOverwrite(ProfRefNpi, stagingHiveDB, tabProfRefNpi)

				ProfRefTax <- ProfRefTax(clmProv, clmLineProv, tabProf, tabProfPdlTax, stagingHiveDB)
				WriteyueProfDF <- WriteManagedTblInsertOverwrite(ProfRefTax, stagingHiveDB, tabProfRefTax)
		...
		//	runIDInsert("YUE_2017_PROF_FINAL_REPORT", maxrun_id)
			null
	}
