...
class MLROperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
  ...
  def loadData(): Map[String, DataFrame] = {
    ...
    val factPhrmcyClmSmryQuery = config.getString("query_fact_phrmcy_clm_smry").replaceAll(ConfigKey.sourceDBPlaceHolder, inboundHiveDB).toLowerCase()
    val ddimAgeMnthQuery = config.getString("query_ddim_age_mnth").replaceAll(ConfigKey.sourceDBPlaceHolder, inboundHiveDB).toLowerCase()
    ...
    println(s"[NDO-ETL] Query for reading data from factPhrmcyClmSmryQuery table is $factPhrmcyClmSmryQuery")
    println(s"[NDO-ETL] Query for reading data from factPhrmcyClmSmryQuery table is $ddimAgeMnthQuery")
    ...
    val ddimAgeMnthDf = spark.sql(ddimAgeMnthQuery)
    
    val factPhrmcyClmSmry = spark.sql(factPhrmcyClmSmryQuery)
    ...
    val mapDF = Map("fact_rsttd_mbrshp" -> factRsttdMbrshpDf, "fact_rsttd_rvnu" -> factRsttdRvnuDf, "fact_rsttd_expns" -> factRsttdExpnsDf,
      ...
      "ddim_bnft_plan" -> ddimBnftPlanDf, "ddim_prov_tax_id" -> ddimProvTaxIdDf, "ddim_age_mnth" -> ddimAgeMnthDf, "ddim_hcc" -> ddimHccDf, "fact_phrmcy_clm_smry" -> factPhrmcyClmSmry)
  ...
  def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
    ...
    val ddimAgeMnthDf = inMapDF.getOrElse("ddim_age_mnth", null)
    val lastUpdatedDate = config.getString(ConfigKey.auditColumnName)
    val factPhrmcyClmSmry = inMapDF.getOrElse("fact_phrmcy_clm_smry", null)
    ...
    val tabndoPhrmcyClmSmry = config.getString("tab_ndo_phrmcy_clm_smry")
    ...
    val factPhrmcyClmSmryFil = factPhrmcyClmSmry.filter(factPhrmcyClmSmry("SNAP_YEAR_MNTH_NBR") === snapNbr &&
      (factPhrmcyClmSmry("ANLYTC_INCRD_YEAR_MNTH_NBR").between(priorBeginYyyymm, priorEndYyyymm) || factPhrmcyClmSmry("ANLYTC_INCRD_YEAR_MNTH_NBR").
        between(currentBeginYyyymm, currentEndYyyymm)))

    val ddimProvTaxIdFil = ddimProvTaxIdDf.filter(ddimProvTaxIdDf("SNAP_YEAR_MNTH_NBR") === snapNbr)
    
     val factRsttdMbrshpFil3 = factRsttdMbrshpDf.filter(factRsttdMbrshpDf("SNAP_YEAR_MNTH_NBR") === snapNbr &&
      (factRsttdMbrshpDf("ANLYTC_INCRD_YEAR_MNTH_NBR").between(currentBeginYyyymm, currentEndYyyymm)))
    ...
    val f = for {
      ...
      mlrMbrDf <- mlrMbr(factRsttdMbrshpFil, tabMlrSvcArea, tabMlrBenPlan, tabMlrHcc,ddimAgeMnthDf, priorBeginYyyymm, priorEndYyyymm, currentBeginYyyymm, currentEndYyyymm, priorPeriod, currentPeriod, stagingHiveDB)
     ...
     ndoPhrmcyClmSmryDf <- ndoPhrmcyClmSmry(factPhrmcyClmSmryFil, tabMlrSvcArea, tabMlrBenPlan, tabMlrHcc, priorBeginYyyymm, priorEndYyyymm,currentBeginYyyymm, currentEndYyyymm, priorPeriod, currentPeriod, stagingHiveDB)
     WriteNdoPhrmcyClmSmryDf <- WriteManagedTblInsertOverwrite(ndoPhrmcyClmSmryDf, stagingHiveDB, tabndoPhrmcyClmSmry)

      mlrAllprovDf <- mlrAllprov(tabmlrMbr, mlrMbrRankDf, tabndoPhrmcyClmSmry, stagingHiveDB)
     ...
     mlrAgeMedianDf <- mlrAgeMedian(factRsttdMbrshpFil3, tabMlrSvcArea, tabMlrBenPlan, tabMlrHcc, ddimAgeMnthDf,priorBeginYyyymm, priorEndYyyymm,currentBeginYyyymm, currentEndYyyymm, priorPeriod, currentPeriod, stagingHiveDB)
		 WritemlrAgeMedian <- WriteManagedTblInsertOverwrite(mlrAgeMedianDf, stagingHiveDB, tabMlrAgeMedian)
      ...
      mlrDataSmry2Df <- mlrDataSmry2(tabApiMlrDtl, warehouseHiveDB, tabMlrAgeMedian, mlrDataSmryDf , currentPeriod,stagingHiveDB)
      WriteMlrBenPlan <- WriteExternalTblAppend(mlrDataSmry2Df, stagingHiveDB, tabMlrDataSmry)
  ...
  def getSnapNbr(inMapDF: Map[String, DataFrame]): Long = {
    ...
    val ddimAgeMnthDf = inMapDF.getOrElse("ddim_age_mnth", null).select($"SNAP_YEAR_MNTH_NBR").distinct
    val factPhrmcyClmSmry = inMapDF.getOrElse("fact_phrmcy_clm_smry", null).select($"SNAP_YEAR_MNTH_NBR").distinct
    ...
    val snapNbr = factRsttdMbrshpDf.join(factRsttdRvnuDf, Seq("SNAP_YEAR_MNTH_NBR"))
      ...
      .join(ddimHccDf, Seq("SNAP_YEAR_MNTH_NBR"))
      .join(ddimAgeMnthDf, Seq("SNAP_YEAR_MNTH_NBR"))
      .join(factPhrmcyClmSmry, Seq("SNAP_YEAR_MNTH_NBR")).orderBy($"SNAP_YEAR_MNTH_NBR".desc).head.getLong(0)
