...
class ApiEtgGrpOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	val tabApiEtgGrp = config.getString("tab_api_etg_grp")
	val tabApiEtgGrpBkp = config.getString("tab_api_etg_grp_bkp")
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			val startTime = DateTime.now
					...
					val maxrun_id_api_etg_grp = getMAxRunId(tabApiEtgGrp)
					println(s"[NDO-ETL] Max run id is : $maxrun_id_api_etg_grp")

					val snapNbr = getSnapNbr(inMapDF)
					println(s"[NDO-ETL] Available snapNbr is : $snapNbr")

					val pcrGrpEfcncyRatioMultiSpltDfFil = pcrGrpEfcncyRatioMultiSpltDf.filter(trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("UNK", "NA") === false
					&& pcrGrpEfcncyRatioMultiSpltDf("BKBN_SNAP_YEAR_MNTH_NBR") === snapNbr
					&& pcrGrpEfcncyRatioMultiSpltDf("GRP_DSTNCT_EPSD_CNT") >= 20
					&& trim(pcrGrpEfcncyRatioMultiSpltDf("PHYSN_TYPE_CD")) === "All Physicians"
					&& ((trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("CA", "NY", "VA") && trim(pcrGrpEfcncyRatioMultiSpltDf("PEER_MRKT_CD")).isin("8", "33"))
							|| (trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")).isin("CA", "NY", "VA") === false && trim(pcrGrpEfcncyRatioMultiSpltDf("PEER_MRKT_CD")).isin("4", "32"))
							|| (trim(pcrGrpEfcncyRatioMultiSpltDf("NTWK_ST_CD")) === "NY" and trim(pcrGrpEfcncyRatioMultiSpltDf("PEER_MRKT_CD")).isin("3", "34"))))

					val apiProvWrkDfDist = apiProvWrkDf.select(
							$"ST_CD",
							$"LOB_ID",
							$"PROD_ID",
							$"PROV_TAX_ID",
					...
					val apiProvWrkPcrGrpJoin = pcrGrpEfcncyRatioMultiSpltDfFil.join(apiProvWrkDfDist,
							trim(pcrGrpEfcncyRatioMultiSpltDfFil("GRP_AGRGTN_TYPE_CD")) === trim(apiProvWrkDfDist("AGRGTN_TYPE_CD"))
							&& trim(pcrGrpEfcncyRatioMultiSpltDfFil("GRP_AGRGTN_ID")) === (when(trim(apiProvWrkDfDist("AGRGTN_TYPE_CD")) === "TIN", trim(apiProvWrkDfDist("PROV_TAX_ID")))
									when (trim(apiProvWrkDfDist("AGRGTN_TYPE_CD")) === "NPI", trim(apiProvWrkDfDist("NPI"))) otherwise lit("0"))
							&& trim(pcrGrpEfcncyRatioMultiSpltDfFil("NTWK_ST_CD")) === trim(apiProvWrkDfDist("ST_CD"))
							&& trim(pcrGrpEfcncyRatioMultiSpltDfFil("PEER_MRKT_CD")) === trim(apiProvWrkDfDist("PEER_MRKT_CD"))
							&& trim(apiProvWrkDfDist("LOB_ID")) === (when(trim(pcrGrpEfcncyRatioMultiSpltDfFil("PCR_LOB_DESC")) === "MEDICARE", lit("MEDICARE ADVANTAGE")) otherwise trim(pcrGrpEfcncyRatioMultiSpltDfFil("PCR_LOB_DESC")))
							&& trim(pcrGrpEfcncyRatioMultiSpltDfFil("PRMRY_SUBMRKT_CD")) === trim(apiProvWrkDfDist("SUBMRKT_CD"))
							&& ((trim(apiProvWrkDfDist("PEER_MRKT_CD")).isin("32", "33", "34") && trim(apiProvWrkDfDist("RPTG_NTWK_DESC")) === trim(pcrGrpEfcncyRatioMultiSpltDfFil("BNCHMRK_PROD_DESC")))
									|| (trim(apiProvWrkDfDist("PEER_MRKT_CD")).isin("3", "4", "8") &&
											trim(apiProvWrkDfDist("prod_id")) === (when(trim(pcrGrpEfcncyRatioMultiSpltDfFil("BNCHMRK_PROD_DESC")) === "NO GROUPING", lit("ALL")) otherwise trim(pcrGrpEfcncyRatioMultiSpltDfFil("BNCHMRK_PROD_DESC"))))),
							"inner").select(
									pcrGrpEfcncyRatioMultiSpltDfFil("BKBN_SNAP_YEAR_MNTH_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("NTWK_ST_CD").alias("ST_CD"),
									(when(trim(pcrGrpEfcncyRatioMultiSpltDfFil("PCR_LOB_DESC")) === "MEDICARE", lit("MEDICARE ADVANTAGE")) otherwise trim(pcrGrpEfcncyRatioMultiSpltDfFil("PCR_LOB_DESC"))).alias("LOB_ID"),
									apiProvWrkDfDist("PROD_ID"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_AGRGTN_ID").alias("AGRGTN_ID"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_AGRGTN_TYPE_CD").alias("AGRGTN_TYPE_CD"),
									apiProvWrkDfDist("SPCLTY_ID").alias("SPCLTY_ID"),
									pcrGrpEfcncyRatioMultiSpltDfFil("PRMRY_SUBMRKT_CD").alias("SUBMRKT_CD"),
									apiProvWrkDfDist("PROV_CNTY_NM").alias("PROV_CNTY_NM"),
									(when(apiProvWrkDfDist("PEER_MRKT_CD").isin("3", "4", "8"), lit("NONE"))
											when (apiProvWrkDfDist("PEER_MRKT_CD").isin("32", "33", "34"), pcrGrpEfcncyRatioMultiSpltDfFil("BNCHMRK_PROD_DESC"))
											otherwise lit("NOTF")).alias("RPTG_NTWK_DESC"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_DSTNCT_EPSD_CNT").alias("GRP_EPSD_VOL_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_SCRBL_AVG_EPSD_OE_VOL_WGTD_NBR").alias("GRP_VOL_WGTD_MEAN_ETG_INDX_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_OE_LOWR_CNFDNC_LVL_VOL_WGTD_NBR").alias("GRP_90_PCT_LOWR_CL_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_OE_UPR_CNFDNC_LVL_VOL_WGTD_NBR").alias("GRP_90_PCT_UPR_CL_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_SCRBL_AVG_EPSD_NRMLZD_OE_VOL_WGTD_NBR").alias("GRP_NRMLZD_VOL_WGTD_MEAN_ETG_INDX_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_OE_LOWR_CNFDNC_LVL_NRMLZD_VOL_WGTD_NBR").alias("GRP_NRMLZD_90_PCT_LOWR_CL_NBR"),
									pcrGrpEfcncyRatioMultiSpltDfFil("GRP_OE_UPR_CNFDNC_LVL_NRMLZD_VOL_WGTD_NBR").alias("GRP_NRMLZD_90_PCT_UPR_CL_NBR")).distinct()

					val apiEtgGrpDf = apiProvWrkPcrGrpJoin.withColumn("RCRD_CREATN_DTM", lit(current_timestamp())).
					...
					.withColumn("run_id", lit(maxrun_id_api_etg_grp))

					writeTblInsertOverwrite(apiEtgGrpDf, warehouseHiveDB, tabApiEtgGrp)
					writeTblAppendOverwrite(apiEtgGrpDf, warehouseHiveDB, tabApiEtgGrpBkp)

					runIDInsert(tabApiEtgGrp, maxrun_id_api_etg_grp)
