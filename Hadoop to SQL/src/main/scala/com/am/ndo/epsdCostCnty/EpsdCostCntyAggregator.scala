...
//and trim(col("pcrCostEpsdDf1.PEER_MRKT_CD")) filter change to single peer_mrkt_cd
			...
			val pcrJoinApi = pcrCostEpsdJoin.join(pcrEtgMbrSmryDf, pcrEtgMbrSmryDf("mcid") === pcrCostEpsdDf("mcid")
...
//and trim(col("pcrCostEpsdDf1.PEER_MRKT_CD")) filter change to single peer_mrkt_cd
			...
			val finalSel = pcrApiJoinPcrSmry.select(
					...
					(when((when(pcrEtgMbrSmryDf("MBR_RSDNT_ST_CD").!==("NA"), pcrEtgMbrSmryDf("MBR_RSDNT_ST_CD")).otherwise(coalesce(mlsaZipCdDfFil("ST_CD"), lit("UNK")))).!==(pcrCostEpsdJoin("NTWK_ST_CD")),lit("UNK")) otherwise(coalesce(ndoZipSubmarketXwalkDf("CNTY_NM"), lit("UNK")))).alias("MBR_COUNTY"),
			...
			import spark.implicits._
					...
					$"EPSD_NBR").groupBy(
					    ...
					    $"RPTG_NTWK_DESC").agg(
									...
									(sum($"EPSD_TOTL_ANLZD_ALWD_AMT") ).alias("EPSD_TOTLS_ANLZD_ALWD_AMT"),
									(sum($"PEER_GRP_AVG_ANLZD_ALWD_AMT") ).alias("PEER_GRP_EXPCTD_ANLZD_ALWD_AMT"),
									(sum($"EPSD_NO_FI_TOTL_ANLZD_ALWD_AMT") ).alias("EPSD_NO_FI_TOTLS_ANLZD_ALWD_AMT"),
									(sum($"PEER_GRP_AVG_ANLZD_NOFI_AMT") ).alias("PEER_NO_FI_GRP_EXPCTD_ANLZD_AMT")).select(
							$"BKBN_SNAP_YEAR_MNTH_NBR",
							$"NTWK_ST_CD",
							$"PCR_LOB_DESC",
							$"PROD_ID",
