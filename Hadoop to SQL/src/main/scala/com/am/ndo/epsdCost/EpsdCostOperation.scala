...
class EpsdCostOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
	...
	def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
			val startTime = DateTime.now
					...
					val tabNdoWrkEpsdCost2 = config.getString("tab_ndowrk_epds_cost2")
					val tabNdoWrkEpsdCost = config.getString("tab_ndowrk_epsd_cost")
					val tabNdoWrkEpsdCostBkp = config.getString("tab_ndowrk_epsd_cost_bkp")

					val maxrun_id = getMAxRunId(tabNdoWrkEpsdCost)
...
//						WriteNdoWrkEpsdCost2 <- WriteSaveAsTableOverwrite(ndoWrkEpsdCost2Df, stagingHiveDB, tabNdoWrkEpsdCost2)
						...
						WriteMlrBenPlan <- WriteManagedTblInsertOverwrite(ndoWrkEpsdCostDfWc, warehouseHiveDB, tabNdoWrkEpsdCost)
						WriteapiMlrSmryWC <- WriteExternalTblAppend(ndoWrkEpsdCostDfWc, warehouseHiveDB, tabNdoWrkEpsdCostBkp)

					} yield {
						println("Episode Cost - Monad execution completed!")
					}
					f.run(spark)
					runIDInsert(tabNdoWrkEpsdCost, maxrun_id)
