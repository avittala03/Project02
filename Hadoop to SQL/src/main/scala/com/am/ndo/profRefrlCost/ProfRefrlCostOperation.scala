package com.am.ndo.profRefrlCost
...
class ProfRefrlCostOperation(configPath: String, env: String, queryFileCategory: String) extends OperationSession(configPath, env, queryFileCategory) with Operator {
  ...
  val tabProfRfrlCost = config.getString("tab_ndo_prof_rfrl_cost ")
  val tabProfRfrlCostBkp = config.getString("tab_ndo_prof_rfrl_cost_bkp")
  ...
  def processData(inMapDF: Map[String, DataFrame]): Map[String, DataFrame] = {
    ...
    val maxrun_id_ProfRfrlCost = getMAxRunId(tabProfRfrlCost)
    
    println(s"[NDO-ETL] Max run id is : $maxrun_id_ProfRfrlCost")
    ...
    val pcrEtgClmOutptFil = pcrEtgClmOutptDf.filter(pcrEtgClmOutptDf("ETG_RUN_ID") === maxEtgRunId && trim(pcrEtgClmOutptDf("RCRD_TYPE_CD")).isin("A", "M","S")
      && trim(pcrEtgClmOutptDf("TOS_LVL_0_CD")).isin("PROF") && trim(pcrEtgClmOutptDf("SRVC_RNDRG_TYPE_CD")).isin("PHYSN", "PANCL"))
    ...
    val FnclClmOutptJoin = pcrEtgClmOutptFil.join(pcrEostEtgJoin, pcrEtgClmOutptFil("EPSD_NBR") === pcrEostEtgJoin("EPSD_NBR") &&
      ...
      .select(
          ...
          pcrEostEtgJoin("GRP_AGRGTN_ID").alias("PRMRY_PROV_TAX_ID"),
          (when( substring(trim($"INPT_PROV_ID_CMBNTN_TYPE_NM"),1,3) === "TAX", substring(trim($"ETG_INPT_PROV_ID"), 1, 9 ))
              otherwise $"RSPNSBL_EDW_TAX_ID").alias("SCNDRY_PROV_TAX_ID"),            
          ...
          pcrEtgClmOutptFil("MNGMNT_ALWD_AMT"),
          pcrEtgClmOutptFil("SRGRY_ALWD_AMT"),
          ...
          pcrEtgClmOutptFil("EPSD_NBR")).groupBy(
            ...
            agg(
              ...
              sum(pcrEtgClmOutptFil("MNGMNT_ALWD_AMT")).alias("MNGMNT_ALWD_AMT"),
              sum(pcrEtgClmOutptFil("SRGRY_ALWD_AMT")).alias("SRGRY_ALWD_AMT"),
      ...
      .withColumn("LAST_UPDT_USER_ID", lit(sc.sparkUser)).withColumn("run_id", lit(maxrun_id_ProfRfrlCost))

 

    runIDInsert(tabProfRfrlCost, maxrun_id_ProfRfrlCost)

    val mapDF = Map(tabProfRfrlCost -> FnclClmOutptJoin)
  ...
  def writeData(map: Map[String, DataFrame]): Unit = {
    ...
    val ProfRfrlCostDf = map.getOrElse(tabProfRfrlCost, null)

    println(s"[NDO-ETL] Write started for the table Prof Rfrl at:" + DateTime.now())
    ProfRfrlCostDf.write.mode("overwrite").insertInto(warehouseHiveDB + """.""" + tabProfRfrlCost)
    println(s"[NDO-ETL] Write started for the table Prof Rfrl Bkp at:" + DateTime.now())
    ProfRfrlCostDf.write.mode("append").insertInto(warehouseHiveDB + """.""" + tabProfRfrlCostBkp)
