#====================================================================================================
#!/bin/sh
# Title            : NDO_PCR_SUBSPCLTY
# ProjectName      : NDO
# Filename         : NDO_PCR_SUBSPCLTY.sh
# Developer        : Anthem
# Created on       : MAY 2019
# Location         : ATLANTA
# Date           Auth           Ver#     Modified By(Name)                 Change and Reason for Change
# ----------    -----        -----------------------------               --------------------------------------
# 2019/29/05     Legato         1     Initial Version
#====================================================================================================
#=================================================================================================================
...
#VARIABLES:
DRIVER_CLASS="PcrSubSpcltyDriver"
...
echo "$(v_TIMESTAMP):INFO:NDO PCR SUBSPCLTY -$SUBJECT_AREA Script logs in  $LOG_FILE"
echo "$(v_TIMESTAMP):INFO:NDO PCR SUBSPCLTY wrapper triggered">>$LOG_FILE
...
$v_SPARK_SUBMIT_2_2 --master yarn --queue ndo_coca_yarn --deploy-mode cluster --name $SUBJECT_AREA --executor-memory 40G --executor-cores 4 --driver-cores 10 --driver-memory 16G --conf spark.sql.codegen.wholeStage=true --conf spark.yarn.maxAppAttempts=1  --conf spark.yarn.driver.memoryOverhead=2048  --conf spark.sql.parquet.cacheMetadata=false --conf spark.yarn.executor.memoryOverhead=10096 --conf spark.network.timeout=900 --conf spark.driver.maxResultSize=0 --conf spark.kryoserializer.buffer.max=1024m --conf spark.rpc.message.maxSize=1024 --conf spark.sql.broadcastTimeout=5800 --conf spark.executor.heartbeatInterval=30s --conf spark.dynamicAllocation.executorIdleTimeout=90 --conf spark.dynamicAllocation.initialExecutors=10 --conf spark.dynamicAllocation.maxExecutors=90 --conf spark.dynamicAllocation.minExecutors=30 --conf spark.sql.autoBroadcastJoinThreshold=604857600 --conf spark.sql.cbo.enabled=true --conf spark.sql.shuffle.partitions=2100 --files hdfs://${HDFS_DIR}//log4j.xml,/etc/spark2/conf.cloudera.spark2_on_yarn/yarn-conf/hive-site.xml --driver-java-options "-Dlog4j.configuration=log4j.xml" --conf "spark.executor.extraJavaOptions=-Dlog4j.configuration=log4j.xml" --class com.am.ndo.pcrSubSpclty.$DRIVER_CLASS hdfs://${HDFS_DIR}/jar/${v_NDO_ETL_LATEST_jar} hdfs://${HDFS_DIR} ${v_ENV} ndo >$YARN_LOG_SMRY_FILE 2>&1
