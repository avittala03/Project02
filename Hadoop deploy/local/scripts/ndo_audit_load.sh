...
exec
$v_SPARK_SUBMIT_2_2 --master yarn --queue ndo_coca_yarn --deploy-mode cluster --name $SUBJECT_AREA --executor-memory 8G --executor-cores 4 --driver-cores 10 --driver-memory 8G --conf spark.sql.shuffle.partitions=2100 --conf spark.sql.codegen.wholeStage=true --conf spark.yarn.maxAppAttempts=1  --conf spark.yarn.driver.memoryOverhead=4096  --conf spark.sql.parquet.cacheMetadata=false --conf spark.yarn.executor.memoryOverhead=8192 --conf spark.network.timeout=900 --conf spark.driver.maxResultSize=0 --conf spark.kryoserializer.buffer.max=1024m --conf spark.rpc.message.maxSize=1024 --conf spark.sql.broadcastTimeout=5800 --conf spark.executor.heartbeatInterval=30s --conf spark.dynamicAllocation.executorIdleTimeout=90 --conf spark.dynamicAllocation.initialExecutors=5 --conf spark.dynamicAllocation.maxExecutors=50 --conf spark.dynamicAllocation.minExecutors=5 --conf spark.sql.autoBroadcastJoinThreshold=6048576 --conf spark.sql.cbo.enabled=true  --files hdfs://${HDFS_DIR}//log4j.xml,/etc/spark2/conf.cloudera.spark2_on_yarn/yarn-conf/hive-site.xml --driver-java-options "-Dlog4j.configuration=log4j.xml" --conf "spark.executor.extraJavaOptions=-Dlog4j.configuration=log4j.xml" --jars /usr/lib/tdch/1.7/lib/terajdbc4.jar,/usr/lib/tdch/1.7/lib/tdgssconfig.jar --class com.am.ndo.auditTable.${DRIVER_CLASS} hdfs://${HDFS_DIR}/jar/${v_NDO_ETL_LATEST_jar} hdfs://${HDFS_DIR} ${v_ENV}_gbd ndo >$YARN_LOG_SMRY_FILE 2>&1
