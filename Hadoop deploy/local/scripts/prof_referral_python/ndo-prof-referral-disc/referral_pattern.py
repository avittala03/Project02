...
class RefPattern(object):
    ...
    def load_data(self, spark, load_known_ref_pair=False):
        """load data"""
        self.__load_param()
        self.__load_dist_data(spark)
        self.__load_claim_data(spark, load_known_ref_pair=False)
    ...
    def __load_dist_data(self, spark):
        ...
        self.dist = spark.createDataFrame(pd_dist)
    ...
    def __load_claim_data(self, spark, load_known_ref_pair=False):
        """
    ...
    def claim_data_stats(self, spark, check_missing = True):
        """
    ...
    def validate_referral2(self, spark, space_cond=True):
        """
    ...
    def __create_outer_loop_df(self, spark):
        """
    ...
    def __imp_cnt_match_space2(self, spark, df_outer):
        """
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_zip_dist".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_zip_dist as select * from distanceTable".format(DB=self.intermediate_db_name))
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_ini_space".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_ini_space as select * from baseTable".format(DB=self.intermediate_db_name))
        ...
        df_valid = spark.sql("select * from {DB}.ndo_ref_ini_space".format(DB=self.intermediate_db_name))
    ...
    def combine_imp_cnt_across_rule(self, spark, time_wt=0.351, zip_wt=0.277, diag_wt=0.256, org_wt=0.116, overall_wt=1.0):
        """
        ...
        df_outer = self.__create_outer_loop_df(spark)

        # inner loop
        time_cnt = self.__imp_cnt_match_time(df_outer)
        time_cnt.createOrReplaceTempView("tempTable")
        spark.sql("drop table if exists {DB}.ndo_ref_time_cnt".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_time_cnt as select * from tempTable".format(DB=self.intermediate_db_name))
        ...
        space_cnt = self.__imp_cnt_match_space2(spark, df_outer)
        space_cnt.createOrReplaceTempView("tempTable")
        spark.sql("drop table if exists {DB}.ndo_ref_space_cnt".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_space_cnt as select * from tempTable".format(DB=self.intermediate_db_name))
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_diag_cnt".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_diag_cnt as select * from tempTable".format(DB=self.intermediate_db_name))
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_org_cnt".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_org_cnt as select * from tempTable".format(DB=self.intermediate_db_name))
        ...
        time_cnt = spark.sql("select * from {DB}.ndo_ref_time_cnt".format(DB=self.intermediate_db_name))
        space_cnt = spark.sql("select * from {DB}.ndo_ref_space_cnt".format(DB=self.intermediate_db_name))
        diag_cnt = spark.sql("select * from {DB}.ndo_ref_diag_cnt".format(DB=self.intermediate_db_name))
        org_cnt = spark.sql("select * from {DB}.ndo_ref_org_cnt".format(DB=self.intermediate_db_name))
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_PhaseI".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_PhaseI as select * from tempTable".format(DB=self.intermediate_db_name))
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_mod_imp".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_mod_imp as select * from tempTable".format(DB=self.intermediate_db_name))
        ref_logger.info("NDO_REF_MOD_IMP table created!")
        return mod_imp


    def tune_mod_imp(self, spark, tune_spclty = False, tune_PCP = True):
        ...
        mod_imp = spark.sql("select * from {DB}.ndo_ref_mod_imp".format(DB=self.intermediate_db_name))
        ...
        if tune_PCP:
            ...
            known_count1 = spark.sql(query1).count()
            ...
            query2 = """select *
                        ...
                        """.format(PCP=pcp)
            known_count2 = spark.sql(query2).count()
            ...
            query3 = """select *
                        ...
                        ren_npi_ep_spclty_desc in {PCP}
                        """.format(PCP=pcp, PCP1=pcp1)
            known_count3 = spark.sql(query3).count()
            ...
            query4 = """select *
                        ...
                        ren_npi_ep_spclty_desc not in {PCP1}
                        """.format(PCP1=pcp1)
            known_count4 = spark.sql(query4).count()
        ...
        spark.sql("drop table if exists {DB}.ndo_ref_mod_imp_adj".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_mod_imp_adj as select * from tempTable".format(DB=self.intermediate_db_name))
        ref_logger.info("Implicit MOD tuning completed!")
        return mod_imp_adj


    def generate_exp_cnt(self, spark):
        """
        ...
        query = """select  
                   ...
                   , ren_npi_ep_spclty_desc
                    """
        mod_exp = spark.sql(query)
        self.mod_exp = mod_exp.select('*')
        mod_exp.createOrReplaceTempView("tempTable")
        spark.sql("drop table if exists {DB}.ndo_ref_exp".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_exp as select * from tempTable".format(DB=self.intermediate_db_name))
        ref_logger.info("exp_cnt table created")
        return mod_exp

    def generate_tot_cnt(self, spark):
        """
        ...
        query = """select  
                   ...
                   , ren_npi_ep_spclty_desc
                    """
        mod_tot = spark.sql(query)
        mod_tot.createOrReplaceTempView("tempTable")
        spark.sql("drop table if exists {DB}.ndo_ref_tot".format(DB=self.intermediate_db_name))
        spark.sql("create table {DB}.ndo_ref_tot as select * from tempTable".format(DB=self.intermediate_db_name))
        ref_logger.info("Done generate_tot_cnt()")
        return mod_tot


    def generate_MOD(self, spark):
        """
        ...
        """
        imp_cnt = self.combine_imp_cnt_across_rule(spark)
        self.generate_exp_cnt(spark)
        self.tune_mod_imp(spark)
        tot_cnt = self.generate_tot_cnt(spark)

        # To prevent GC overhead limit and out of memory, read from Hive
        imp_cnt = spark.sql("select * from {DB}.ndo_ref_mod_imp_adj".format(DB=self.intermediate_db_name))
        exp_cnt = spark.sql("select * from {DB}.ndo_ref_exp".format(DB=self.intermediate_db_name))
        tot_cnt = spark.sql("select * from {DB}.ndo_ref_tot".format(DB=self.intermediate_db_name))
        ...
        self.MAD.createOrReplaceTempView("MAD")
        SNAP_YEAR_MNTH_NBR = spark.sql("select distinct SNAP_YEAR_MNTH_NBR from MAD").collect()[0][0]
        MOD = MOD.withColumn("SNAP_YEAR_MNTH_NBR", lit(SNAP_YEAR_MNTH_NBR) )

        MOD.createOrReplaceTempView("tempTable")
        spark.sql("drop table if exists {output}".format(output=self.out_hive) )
        spark.sql("create table {output} as select * from tempTable".format(output=self.out_hive))
...
def main():
    ...
    # Setup Spark
    conf, spark = setup_pyspark(conf_json)
    ref_pat = RefPattern(conf)

    # Load data
    ref_pat.load_data(spark)
    ref_pat.impute_MAD()

    # Run model
    #ref_pat.claim_data_stats(spark, check_missing=True)
    #ref_pat.validate_referral2(spark)
    #ref_pat.combine_imp_cnt_across_rule(spark)
    #ref_pat.tune_mod_imp(spark)
    #ref_pat.generate_exp_cnt(spark)
    #ref_pat.generate_tot_cnt(hc)
    MOD = ref_pat.generate_MOD(spark)
    MOD.show(100)
