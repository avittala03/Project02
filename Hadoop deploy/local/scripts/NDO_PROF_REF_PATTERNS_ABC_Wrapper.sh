...
#VARIABLES:
SUBJECT_AREA="NDO_PROF_REF_PATTERNS_ABC"
DRIVER_CLASS="ProfReferralPatternsDriverABC"
status="SUCCEEDED"

#Creating log file
LOG_FILE=$LOG_DIR/"script_"$SUBJECT_AREA"_"$(v_DATETIME)"_"$USER.log
#NDO_LOG_FILE=$LOG_DIR/"script_"$SUBJECT_AREA"_"$(v_DATETIME)"_"$USER.log
YARN_LOG_SMRY_FILE=$LOG_FILE_PATH/$SUBJECT_AREA"_"$(v_DATETIME)"_"$USER.log

echo "$(v_TIMESTAMP):INFO:Professional Referral Patterns -$SUBJECT_AREA Script logs in  $LOG_FILE"
echo "$(v_TIMESTAMP):INFO:NDO PROFESSIONAL REFERRAL PATTERNS wrapper triggered">>$LOG_FILE
...
#path to log file
yarn logs -applicationId ${application_name} >
echo "$(v_TIMESTAMP):INFO:Yarn Log file : $YARN_LOG_FILE"  >>$LOG_FILE
...
then 
echo "$(v_TIMESTAMP):INFO: Spark Job for "$SUBJECT_AREA" COMPLETED." >>$NDO_LOG_FILE
rm $TMP_YARN_LOG_FILE
exit $v_SUCCESS
else
echo "$(v_TIMESTAMP):INFO: Spark Job Failed for "$SUBJECT_AREA"" >>$NDO_LOG_FILE
exit $v_ERROR
fi
