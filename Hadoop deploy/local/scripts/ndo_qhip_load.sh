...
{
if [ $1 -eq '0' ];then
       echo "$(v_TIMESTAMP):INFO:$SUBJECT_AREA - TDCH Teradata extract for worktbl_ndo_qhip Completed with RC $1"
else
       echo "$(v_TIMESTAMP):ERROR:$SUBJECT_AREA - TDCH Teradata extract for worktbl_ndo_qhip failed with RC $1"
       exit $v_ERROR
fi
}

sh -x $SCRIPTS_DIR/EXTRACT_AVRO_SCHEMA.sh "DL_PM_PRJ_NDO" "worktbl_ndo_qhip" >>$LOG_FILE
#v_RC_CHECK $?

if [ $? -ne 0 ];then

echo  "The Avro Extract Failed for "DL_PM_PRJ_NDO.worktbl_ndo_qhip" "  >>$LOG_FILE
exit $v_ERROR
else
echo "The Avro Extract completed for "DL_PM_PRJ_NDO.worktbl_ndo_qhip" successfully"   >>$LOG_FILE
fi

echo "$(v_TIMESTAMP):INFO:NDO TDCH run triggered">>$LOG_FILE
/bin/sh $SCRIPTS_DIR/TDCH_EHOPPA_TDW.sh "DL_PM_PRJ_NDO" "worktbl_ndo_qhip" "$v_HIVE_INB_NOGBD" "worktbl_ndo_qhip"
write_flag "$?"
