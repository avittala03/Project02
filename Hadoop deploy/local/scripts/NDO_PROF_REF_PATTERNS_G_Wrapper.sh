...
SUBJECT_AREA="NDO_PROF_REF_PATTERNS_G"
...
export JAVA_HOME=/usr/java/latest
exec
spark2-submit --master yarn --queue ndo_coca_yarn --deploy-mode cluster --name $SUBJECT_AREA --executor-memory 80G --executor-cores 4 --driver-cores 10 --driver-memory 80G --conf spark.sql.codegen.wholeStage=true --conf spark.sql.shuffle.partitions=2100 --conf spark.yarn.maxAppAttempts=1  --conf spark.yarn.driver.memoryOverhead=4096  --conf spark.sql.parquet.cacheMetadata=false --conf spark.yarn.executor.memoryOverhead=8192 --conf spark.network.timeout=900 --conf spark.driver.maxResultSize=0 --conf spark.kryoserializer.buffer.max=1024m --conf spark.rpc.message.maxSize=1024 --conf spark.sql.broadcastTimeout=5800 --conf spark.executor.heartbeatInterval=30s --conf spark.dynamicAllocation.executorIdleTimeout=90 --conf spark.dynamicAllocation.initialExecutors=10 --conf spark.dynamicAllocation.maxExecutors=100 --conf spark.dynamicAllocation.minExecutors=10 --conf spark.sql.autoBroadcastJoinThreshold=604857600 --conf spark.sql.cbo.enabled=true  --files hdfs:///ts/hdfsapp/vs2/pdp/pndo/phi/no_gbd/r000/bin/log4j.xml,/etc/spark2/conf.cloudera.spark2_on_yarn/yarn-conf/hive-site.xml --driver-java-options "-Dlog4j.configuration=log4j.xml" --conf "spark.executor.extraJavaOptions=-Dlog4j.configuration=log4j.xml" --jars /usr/lib/tdch/1.5/lib/terajdbc4.jar,/usr/lib/tdch/1.5/lib/tdgssconfig.jar --class com.am.ndo.profReferralPatterns.ProfReferralPatternsDriverG hdfs:///ts/hdfsapp/vs2/pdp/pndo/phi/no_gbd/r000/bin/jar/prof-ref.jar  hdfs:///ts/hdfsapp/vs2/pdp/pndo/phi/no_gbd/r000/bin sit ndo >$SLogFullFileName 2>&1
